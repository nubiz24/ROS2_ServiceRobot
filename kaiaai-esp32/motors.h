...
enum motor_driver_t {
  ...
  MOT_DRIVER_L298_EN, // L298: PWM on ENA/ENB, direction on IN1/IN2
};
...
void setMotorPWM(MotorController *motor_controller, float pwm) {
  ...
  pwm = pwm > 1 ? 1 : pwm;
  pwm = pwm < -1 ? -1 : pwm;
  int pwm_magnitude = round(max_pwm * abs(pwm));
  ...
  switch(motorDriverType) {
    ...
    case MOT_DRIVER_L298_EN:
      if (pwm == 0) {
        // Coast at zero target speed.
...
void setupDriver(motor_driver_t motor_driver_type) {
  ...
  switch(motorDriverType) {
    case MOT_DRIVER_L298_EN:
      ...
      setPinMode(cfg.mot_left_drv_gpio_ena, OUTPUT);
      setPinMode(cfg.mot_right_drv_gpio_enb, OUTPUT);

      #if ESP_IDF_VERSION_MAJOR >= 5
      if (!ledcAttachChannel(cfg.mot_left_drv_gpio_ena, cfg.MOT_PWM_FREQ,
             cfg.MOT_PWM_BITS, cfg.MOT_PWM_LEFT_CHANNEL) ||
          !ledcAttachChannel(cfg.mot_right_drv_gpio_enb, cfg.MOT_PWM_FREQ,
      ...
      ledcAttachPin(cfg.mot_left_drv_gpio_ena, cfg.MOT_PWM_LEFT_CHANNEL);
      ledcAttachPin(cfg.mot_right_drv_gpio_enb, cfg.MOT_PWM_RIGHT_CHANNEL);
      #endif
      break;
...
void setupMotors() {
  ...
  if (cfg.motor_driver_type == "L298_EN") {
    setupDriver(MOT_DRIVER_L298_EN);
  } else if (cfg.motor_driver_type == "PWM_CW") {
...
void setMotorSpeeds(float rpm_left, float rpm_right) {
  ...
  static constexpr float TARGET_RPM_DEADBAND = 2.0f;
