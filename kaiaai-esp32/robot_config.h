...
public:
  ...
  // Match typical L298N + ESP32 tests (e.g. 5 kHz, 8-bit). Too-high freq can make
  // effective speed control poor on some boards; PID still maps [-1,1] to duty.
  static const uint16_t MOT_PWM_FREQ = 5000;
  static const uint8_t MOT_PWM_BITS = 8;
...
public:
  ...
  String motor_driver_type = "L298_EN";
  ...
  float motor_driver_pid_kp = 0.5f;
  float motor_driver_pid_ki = 0.0f;
  float motor_driver_pid_kd = 0.0f;
  ...
  uint8_t mot_left_drv_gpio_ena = 255;
  uint8_t mot_right_drv_gpio_enb = 255;
        ...
        if (nlevels >= 4 && (left || lname[1] == "right")) {
          if (lname[2] == "encoder") {
            ...
            if (nlevels == 4 && lname[3] == "reverse") {
              ...
              } else if (left && (lname[4] == "ena" || lname[4] == "en")) {
                mot_left_drv_gpio_ena = (uint8_t) pvalue.toInt();
              } else if (!left && (lname[4] == "enb" || lname[4] == "en")) {
                mot_right_drv_gpio_enb = (uint8_t) pvalue.toInt();
              }
            }
          }
        }
