...
bool kaiaai_msgs__msg__kaiaai_telemetry__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("kaiaai_msgs.msg._kaiaai_telemetry.KaiaaiTelemetry", full_classname_dest, 49) == 0);
  }
  kaiaai_msgs__msg__KaiaaiTelemetry * ros_message = _ros_message;
  {  // stamp
  ...
  {  // joint_pos
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->joint_pos.data;
  ...
  {  // joint_vel
    ...
    if (PyObject_CheckBuffer(field)) {
      ...
      float * dest = ros_message->joint_vel.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
...
{
  /* NOTE(esteve): Call constructor of KaiaaiTelemetry */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("kaiaai_msgs.msg._kaiaai_telemetry");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "KaiaaiTelemetry");
  ...
  {  // joint_pos
    ...
    if (ros_message->joint_pos.size > 0) {
      ...
      float * src = &(ros_message->joint_pos.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->joint_pos.size * sizeof(float));
      assert(data != NULL);
  ...
  {  // joint_vel
    ...
    if (ros_message->joint_vel.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->joint_vel.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->joint_vel.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
